...
import com.fula.mysticchina.model.VideoInfo
...
import com.fula.mysticchina.util.fileExists
...
internal class MainPage : BasePager() {
    ...
    private fun addTestVideos() {
        videoList.addAll(listOf(
            VideoInfo("real1", "test.mp4", "/data/data/com.fula.mysticchina/files/screen_test.mp4", duration = 3000L),
    ...
    override fun body(): ViewBuilder {
        val ctx = this
        return {
            attr { backgroundColor(MysticChinaColors.background) }
            ...
            View {
                ...
                Text { attr { text("神秘中国"); fontSize(MysticChinaTheme.FontSize.title); color(MysticChinaColors.textPrimary); fontWeightBold() } }
            }
            ...
            List {
                attr { flex(1f) }
                View {
                    attr { flexDirectionRow(); flexWrapWrap(); padding(all = MysticChinaTheme.Spacing.sm) }
                    vfor({ ctx.videoList }) { video ->
                        val cw = ctx.pagerData.pageViewWidth / 2 - MysticChinaTheme.Spacing.md
                        View {
                            attr {
                                size(cw, cw * 9f / 16f + 40f); flexDirectionColumn(); margin(MysticChinaTheme.Spacing.xs)
                                backgroundColor(MysticChinaColors.surface); borderRadius(MysticChinaTheme.Radius.md); overflow(true)
                            }
                            event { click { ctx.onVideoClick(video) } }
                            View {
                                attr { size(cw, cw * 9f / 16f); backgroundColor(MysticChinaColors.backgroundLight); allCenter() }
                                View { attr { size(44f, 44f); borderRadius(22f); backgroundColor(Color(0x80000000)); allCenter() }
                                    Text { attr { text("▶"); fontSize(20f); color(MysticChinaColors.textPrimary) } } }
                                if (video.duration > 0) {
                                    View { attr { absolutePosition(bottom = 6f, right = 6f); padding(left = 6f, right = 6f, top = 2f, bottom = 2f); backgroundColor(Color(0xCC000000)); borderRadius(4f) }
                                        Text { attr { text(video.formattedDuration); fontSize(11f); color(MysticChinaColors.textPrimary) } } }
                                }
                            }
                            View { attr { flex(1f); padding(all = MysticChinaTheme.Spacing.sm); justifyContentCenter() }
                                Text { attr { text(video.title); fontSize(MysticChinaTheme.FontSize.small); color(MysticChinaColors.textPrimary) } } }
                        }
                    }
                }
            }
            ...
            // 底部按钮
            View { attr { height(70f); backgroundColor(MysticChinaColors.surfaceLight); allCenter() }
                Button {
                    attr {
                        size(width = 220f, height = 44f); borderRadius(MysticChinaTheme.Radius.round)
                        ...
                        titleAttr { text("🎬 添加视频"); fontSize(16f); color(MysticChinaColors.textPrimary); fontWeightBold() }
                    }
