...
import com.fula.mysticchina.components.*
...
import com.fula.mysticchina.model.VideoClip
...
private fun ViewContainer<*, *>.EditorTopBar(ctx: EditorPage) {
    View {
        attr {
            ...
            backgroundColor(MysticChinaColors.background); flexDirectionRow(); alignItemsCenter()
            paddingTop(ctx.pagerData.statusBarHeight)
            paddingLeft(MysticChinaTheme.Spacing.md); paddingRight(MysticChinaTheme.Spacing.md)
        }
        View { attr { size(40f, 40f); allCenter() }; event { click { ctx.closePage() } }
            Text { attr { text("←"); fontSize(22f); color(MysticChinaColors.textPrimary) } } }
        View { attr { size(36f, 36f); allCenter(); borderRadius(18f); marginLeft(8f)
                backgroundColor(if (ctx.undoEnabled) MysticChinaColors.surfaceLight else Color(0xFF1F1F1F)) }
            Text { attr { text("↩"); fontSize(17f); color(if (ctx.undoEnabled) MysticChinaColors.textPrimary else MysticChinaColors.textDisabled) } } }
        View { attr { size(36f, 36f); allCenter(); borderRadius(18f); marginLeft(6f)
                backgroundColor(if (ctx.redoEnabled) MysticChinaColors.surfaceLight else Color(0xFF1F1F1F)) }
            Text { attr { text("↪"); fontSize(17f); color(if (ctx.redoEnabled) MysticChinaColors.textPrimary else MysticChinaColors.textDisabled) } } }
        View { attr { flex(1f) } }
        View { attr { height(32f); paddingLeft(18f); paddingRight(18f); borderRadius(16f); allCenter()
                backgroundLinearGradient(Direction.TO_RIGHT, ColorStop(MysticChinaColors.gradientStart, 0f), ColorStop(MysticChinaColors.gradientEnd, 1f)) }
            event { click { /* TODO: 导出逻辑 */ } }
            Text { attr { text("导出"); fontSize(14f); color(MysticChinaColors.textPrimary); fontWeightBold() } } }
    }
}
...
private fun ViewContainer<*, *>.EditorPreview(ctx: EditorPage) {
    ...
    View {
        ...
        vif({ ctrl.playerState != PlayerState.PLAYING }) {
            View { attr { absolutePositionAllZero(); allCenter() }
                View { attr { size(60f, 60f); borderRadius(30f); backgroundColor(Color(0xCC000000)); allCenter() }
                    Text { attr { text(if (ctrl.playerState == PlayerState.COMPLETED) "↻" else "▶"); fontSize(26f); color(MysticChinaColors.textPrimary) } } } }
        }

        // 加载中
        vif({ ctrl.playerState == PlayerState.LOADING }) {
            View { attr { absolutePositionAllZero(); allCenter() }
                Text { attr { text("加载中..."); fontSize(14f); color(MysticChinaColors.textSecondary) } } }
        }
        ...
        View { attr { absolutePosition(bottom = 8f, left = 10f); paddingLeft(6f); paddingRight(6f); paddingTop(2f); paddingBottom(2f); backgroundColor(Color(0x99000000)); borderRadius(4f) }
            Text { attr { text("${ctrl.currentTimeText} / ${ctrl.durationText}"); fontSize(11f); color(MysticChinaColors.textPrimary) } } }
    }
}
...
private fun ViewContainer<*, *>.EditorTimeline(ctx: EditorPage) {
    ...
    View {
        attr { height(86f); backgroundColor(MysticChinaColors.background); flexDirectionColumn(); overflow(false) }

        // 时间刻度
        View { attr { height(16f); flexDirectionRow(); alignItemsCenter(); paddingLeft(halfW) }
            for (i in 0..10) {
                View { attr { width(44f); alignItemsCenter() }
                    Text { attr { text(formatTimeMs(i * 2000L)); fontSize(9f); color(MysticChinaColors.textTertiary) } } }
            }
        }
        ...
        // 播放头中心线
        View { attr { absolutePosition(top = 0f, bottom = 0f, left = halfW - 1f); width(2f); backgroundColor(MysticChinaColors.primary) } }

        // 已播放区域指示
        if (ctrl.durationMs > 0) {
            val playedW = screenW * ctrl.currentProgress
            View { attr { absolutePosition(top = 0f, bottom = 0f, left = 0f); width(playedW.coerceAtMost(screenW)); height(2f); backgroundColor(MysticChinaColors.primary) } }
        }
    }
}
...
private fun ViewContainer<*, *>.SubToolItem(ctx: EditorPage, tool: EditorPage.SubTool) {
    ...
    View { attr { width(62f); height(72f); flexDirectionColumn(); alignItemsCenter(); justifyContentCenter(); marginLeft(2f); marginRight(2f) }
        ...
        View { attr { size(38f, 38f); borderRadius(19f); backgroundColor(Color(0xFF2E2E2E)); allCenter(); marginBottom(5f) }
            Text { attr { text(tool.icon); fontSize(17f); color(MysticChinaColors.textPrimary) } } }
        Text { attr { text(tool.label); fontSize(10f); color(MysticChinaColors.textSecondary); textAlignCenter() } } }
}
...
private fun ViewContainer<*, *>.EditorCategoryBar(ctx: EditorPage) {
    View { attr { height(62f + ctx.pagerData.safeAreaInsets.bottom); backgroundColor(MysticChinaColors.background); flexDirectionColumn() }
...
private fun ViewContainer<*, *>.CategoryItem(ctx: EditorPage, category: EditorPage.EditCategory) {
    ...
    View { attr { width(64f); height(58f); flexDirectionColumn(); alignItemsCenter(); justifyContentCenter() }
        ...
        View { attr { size(42f, 42f); borderRadius(21f); backgroundColor(if (selected) Color(0xFF2A2A2A) else Color(0x00000000)); allCenter(); marginBottom(3f) }
            Text { attr { text(category.icon); fontSize(20f); color(if (selected) MysticChinaColors.primary else MysticChinaColors.textSecondary) } } }
        Text { attr { text(category.label); fontSize(10f); color(if (selected) MysticChinaColors.primary else MysticChinaColors.textTertiary); if (selected) fontWeightSemiBold() else fontWeightNormal() } }
        if (selected) View { attr { size(4f, 4f); borderRadius(2f); backgroundColor(MysticChinaColors.primary); marginTop(1f) } } }
}
