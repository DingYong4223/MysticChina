...
| 模块 | 功能 | 优先级 | 阶段 | Android |
|------|------|--------|------|---------|
| 视频导入 | 本地视频选择、相册访问 | P0 | 一期 | ✅ 完成 |
| 视频预览 | 全屏预览、横竖屏适配 | P0 | 一期 | ✅ 完成 |
| 播放控制 | 播放/暂停、进度拖动、音量控制 | P0 | 一期 | ✅ 完成 |
| 媒体库 | 视频列表展示、缩略图加载 | P0 | 一期 | ✅ 完成 |
...
> **平台策略**: 一期仅实现 Android 端，iOS/macOS 保留接口和 stub 实现。

| 功能 | Android | iOS | macOS | 实现位置 |
|---|---|---|---|---|
| 视频列表/媒体库页面 | ✅ 完整 | ✅ 共享 | ✅ 共享 | `MainPage.kt` (commonMain) |
| 视频选择与加载 | ✅ MediaPlayer | 🔶 stub | 🔶 stub | `AndroidVideoPlayer.kt` |
| 全屏视频预览页面 | ✅ 完整 | ✅ 共享 | ✅ 共享 | `PreviewPage.kt` (commonMain) |
| 播放/暂停控制 | ✅ 真实回调 | 🔶 接口预留 | 🔶 接口预留 | `PlayerController.kt` (commonMain) |
| 进度显示与拖动 | ✅ 定时器驱动 | 🔶 接口预留 | 🔶 接口预留 | `PlayerController` + `ProgressBar` |
| 播放完成循环/停止 | ✅ 完整 | 🔶 接口预留 | 🔶 接口预留 | `PlayerController.replay()` |
| 基础播放器UI | ✅ 完整 | ✅ 共享 | ✅ 共享 | `VideoPlayerView.kt` + `BottomBar.kt` |
| 文件/媒体访问 | ✅ MediaStore | 🔶 PHAsset预留 | 🔶 预留 | `FileBridge.kt` + `AndroidFileBridge.kt` |
| 平台播放器工厂 | ✅ MediaPlayer | 🔶 AVPlayer stub | 🔶 AVPlayer stub | `PlatformPlayerFactory.kt` (expect/actual) |
